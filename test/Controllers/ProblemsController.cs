...
namespace test.Controllers
{
    ...
    public class ProblemsController : ControllerBase
    {
        ...
        [Authorize]
        public async Task<ActionResult<Problem>> CreateTestProblem()
        {
            try
            {
                ...
                var createdProblem = await _problemService.CreateProblemAsync(testProblem);
                return CreatedAtAction(nameof(GetProblem), new { id = createdProblem.Id }, createdProblem);
            }
            catch (Exception ex)
            {
                return StatusCode(500, new { message = "An error occurred while creating the test problem", error = ex.Message });
            }
        }
    }
}
